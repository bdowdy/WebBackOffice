    ...
    With RegList
     ...
     For FA = 0 To NetWork_Data
        ConnectADODB Server_Path(2, FA) & FinancialDb, FinancialDb, DB
              Sql = "Select * from [Audits]" 'Where TRY_CAST([Start Date] AS datetime) + TRY_CAST([start time] AS datetime) Between '" & Format(CDate(CDate(Adate) & " " & GetStartTime), "yyyy-mm-dd hh:nn:ss") & "'"
        ConnectADORS Sql, DB, Tb
        With RegList
...
With RegList
    For R = 1 To .Rows - 1
        ...
        Sql = Sql & " and TRY_CAST([Start Time] AS datetime) = '" & Format(CDate(CDate(.TextMatrix(R, 1))), "yyyy-mm-dd hh:nn:ss") & "'"
